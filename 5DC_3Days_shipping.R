...
for (i in 1:505){
  for (j in 2:16){
    if(transit_time_ground[i,j]>days){
...
for (i in 1:15){
  add.constraint(lps.model3, dc_supply_select[i,], "<=", Supply_select_constraint_num[i])
}
...
total_cost = get.objective(lps.model3)


print(total_cost)
print(results_table_three_days)
########################################################
# which customer zone actually use three days shipping???
...
customerzone_plan = t(results_table_three_days[,4:508])
dim(customerzone_plan)

use_three_days_shipping = outbound_ground_cost
for (i in 1:505){
  for (j in 2:16){
    if(transit_time_ground[i,j]>days){
      use_three_days_shipping[i,j] = 1
    }
    else{
      use_three_days_shipping[i,j] = 0
    }
  }
}
dim(use_three_days_shipping)
head(use_three_days_shipping)

nnn = 0
for (i in 1:505){
  # i = 445
  for (j in 2:16){
    if(use_three_days_shipping[i,j]*customerzone_plan[i,j-1]!=0){
      #print(as.numeric(use_three_days_shipping[i,1]))
      nnn = nnn+1
    }
  }
}
